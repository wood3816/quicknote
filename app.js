(() => {
  ...
  async function finalizeDraft(source="auto"){const content=els.input.value.trim();if(!content){if(source==="manual"){toast("目前沒有內容可建立。");els.input.focus()}return false}setSave("pending","● 儲存中");try{await queueSave();await saveQueue;const n=notes.find(x=>x.id===draftId);if(!n)return false;const now=nowISO();n.content=els.input.value;n.updatedAt=now;n.lastSavedAt=now;n.isDraft=false;n.sortOrder=maxOrder()+1;await dbPut(n);draftId=null;localStorage.removeItem(LS.draftId);localStorage.removeItem(LS.draftShadow);timerPaused=false;timerDeadline=0;remainingMs=C.AUTO_CREATE_MS;persistTimer();els.input.value="";autoGrow(els.input);setSave("ok","✅ 即時儲存");countdownUI();renderNotes();renderCalendar();if(source==="manual")toast("已儲存，已建立新一筆。");setTimeout(()=>els.input.focus({preventScroll:true}),0);return true}catch(e){console.error(e);setSave("error","● 儲存異常");toast("建立新紀錄失敗，輸入內容已保留。");return false}}
  ...
  function attach(){
    ...
    els.navSearch.addEventListener("click",showSearch);els.navCalendar.addEventListener("click",showCalendar);els.navSettings.addEventListener("click",showSettings);
    els.manualCreate.addEventListener("click",()=>finalizeDraft("manual"));
